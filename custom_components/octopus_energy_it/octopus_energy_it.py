...
Includes methods for authentication, fetching account details,
managing devices, and retrieving electricity usage and tariff data.
"""
...
# Global token manager to prevent redundant token requests
...
class OctopusEnergyIT:
    ...
    async def _execute_graphql(
        ...
        *,
        ...
        if (
            ...
            _LOGGER.warning(
                "Token expired during GraphQL request; refreshing and retrying"
            )
    ...
    async def login(self) -> bool:
        ...
        async with self._login_lock:
            ...
            query = """
            mutation krakenTokenAuthentication(
    ...
    async def fetch_all_data(self, account_number: str):
        ...
        try:
            ...
            if LOG_API_RESPONSES:
                ...
                _LOGGER.debug(
                    "API request completed. Set LOG_API_RESPONSES=True for details"
                )
            ...
            if "data" in response:
                ...
                if account_payload:
                    ...
                    electricity_products = self.extract_electricity_products(
                        account_payload
                    )
                ...
                # Log errors but don't fail if we got account data
                if "errors" in response and result["account"]:
                    # Filter errors about missing devices/dispatches
                    non_critical_errors = [
                        error
                    ...
                    if non_critical_errors:
                        if account_number not in self._missing_device_logged_accounts:
                            ...
                            _LOGGER.info(
                                "No devices/dispatches for"
                                " account %s (expected without"
                                " smart devices): %s",
                                account_number,
                                non_critical_errors,
                            )
                            ...
                            _LOGGER.debug(
                                "Still no devices/dispatches for"
                                " account %s (expected without"
                                " smart devices)",
                                account_number,
                            )
    ...
    def build_electricity_product_entry(self, supply_point, agreement):
        ...
        is_time_of_use = any(
            rate is not None for rate in (f2_rate, f3_rate)
        ) or normalised_type in {
            "time_of_use",
            "timeofuse",
            "tou",
        }
    ...
    async def change_device_suspension(self, device_id: str, action: str):
        ...
        if not isinstance(response, dict):
            _LOGGER.error(
                "Invalid response while changing device suspension: %s", response
            )
            return None
        ...
        return response.get("data", {}).get("updateDeviceSmartControl", {}).get("id")
    ...
    async def set_device_preferences(
        ...
        _LOGGER.debug(
            "Setting device preferences: device=%s, target=%s%%, time=%s",
            device_id,
            target_percentage,
            formatted_time,
        )
    ...
    async def update_boost_charge(self, device_id: str, action: str) -> str | None:
        """
        Trigger or cancel a boost charge for a device.

        *action* should be ``"BOOST"`` or ``"CANCEL"``.
        ...
        if not isinstance(response, dict):
            _LOGGER.error(
                "Invalid response for boost charge %s on %s: %s",
                action,
                device_id,
                response,
            )
        ...
        if errors := response.get("errors"):
            error_messages = [(e.get("message") or "Unknown error") for e in errors]
            _LOGGER.error(
                "GraphQL errors for boost charge %s on %s: %s",
                action,
                device_id,
                "; ".join(error_messages),
            )
        ...
        if result is None:
            _LOGGER.error("No result from updateBoostCharge for device %s", device_id)
            return None
    ...
    async def fetch_gas_meter_readings(
        ...
        *,
        ...
        _LOGGER.debug(
            "Fetching gas readings: account=%s,"
            " PDR=%s (first=%s, last=%s,"
            " from=%s, to=%s)",
            account_number,
            pdr,
            first,
        ...
        if not isinstance(response, dict):
            _LOGGER.error(
                "Invalid gas readings response for account %s, PDR %s: %s",
                account_number,
                pdr,
                response,
            )
    ...
    async def fetch_electricity_measurements(
        ...
        *,
        ...
        _LOGGER.debug(
            "Fetching electricity measurements:"
            " property=%s, POD=%s (first=%s,"
            " last=%s, start=%s, end=%s)",
            property_id,
            pod,
            first,
            last,
        ...
        if not isinstance(response, dict):
            _LOGGER.error(
                "Invalid electricity measurements response for property %s: %s",
                property_id,
                response,
            )
            return []
        ...
        try:
            ...
            if isinstance(time_str, str):
                ...
                raise ValueError(
                    ...
                    " Use HH:MM format (e.g. '05:00')"
                )

            raise ValueError(f"Unsupported time value type: {type(time_str).__name__}")
