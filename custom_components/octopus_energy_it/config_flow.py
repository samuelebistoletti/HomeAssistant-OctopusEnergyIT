...
async def validate_credentials(
    ...
    octopus_api = OctopusEnergyIT(email, password)
    try:
...
class OctopusEnergyITConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry):
        ...
        return OctopusEnergyITOptionsFlow()
        ...
        if user_input is not None:
            ...
            if valid:
                ...
                return self.async_create_entry(
                    title=f"Octopus Energy Italy ({email})", data=user_input
                )
...
class OctopusEnergyITOptionsFlow(config_entries.OptionsFlow):
    """Handle an options flow for Octopus Energy Italy."""
